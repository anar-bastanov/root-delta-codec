...
{
    private sealed class ImageTransform_M7_C4 : ImageTransformImpl
    {
        ...
        public override RawImage Decode(RawImage rawImage)
        {
            ...
            for (int s = 0; s < sLen; ++s)
            {
                ...
                for (int p = 1; p < pLen; ++p)
                {
                    ...
                    byte ld  = data[baseL  + off1];
                    byte cod = data[baseCo + off1];
                    byte cgd = data[baseCg + off1];
                    byte ad  = data[baseA  + off1];
